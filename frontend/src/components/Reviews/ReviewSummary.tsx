...
export default function ReviewSummary({ summary }: ReviewSummaryProps) {
  ...
  const noReviews = summary.totalReviews === 0;

  return (
    <div className="bg-[#F6F2E5]/50 rounded-lg p-4 sm:p-6 border border-gray-200">
      <h3 className="text-base sm:text-lg font-semibold text-[#4A5130] mb-3 sm:mb-4">Customer Reviews</h3>

      <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 mb-4 sm:mb-6">
        {/* Average Rating */}
        <div className="text-center sm:text-left flex-shrink-0">
          <div className="text-3xl sm:text-4xl font-bold text-[#4A5130] mb-1">
            {summary.averageRating.toFixed(1)}
          </div>
          <div className="flex justify-center sm:justify-start mb-1">
            <StarRating rating={summary.averageRating} size="md" />
          </div>
          <p className="text-xs sm:text-sm text-[#4A5130] mt-1">
        ...
        {/* Rating Distribution */}
        {noReviews ? (
          <div className="flex-1 text-sm sm:text-base text-gray-500 bg-white border border-dashed border-gray-300 rounded-lg px-4 py-3">
            ...
            {[5, 4, 3, 2, 1].map((star) => {
              ...
              return (
                <div key={star} className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                  <span className="text-xs sm:text-sm text-[#4A5130] w-6 sm:w-8 flex-shrink-0">{star} ★</span>
                  ...
                  <span className="text-xs sm:text-sm text-[#4A5130] w-8 sm:w-10 text-right flex-shrink-0">{count}</span>
                </div>
              );
            })}
          </div>
